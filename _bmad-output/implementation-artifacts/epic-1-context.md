...
- Story 1.9: Shared packing list
- Story 1.10: Reusable-template validation

## Requirements & Constraints

- v1 is one-way: organizer publishes/unlocks, guests only consume. Only three things are ever server-mutable: chapter `unlocked` flags, push subscriptions, and packing-list checked state — all other trip content lives only in static authored content, never the database.
...
- The packing list always renders in full regardless of lock state; concurrent checks/unchecks from different people must resolve deterministically (last-write-wins, no lost updates).
...
- Stack is pinned: Astro (latest 7.x), hand-written web manifest + service worker (no PWA build plugin), PushForge for VAPID Web Push, Vercel Functions on the Node.js runtime, Neon Postgres via the Vercel Marketplace integration (dev/prod branches, no separate staging).
- Two-layer data model: `TripContent` (static, versioned — slug, startDate, accentColor, chapters with id/order/kind/title/time/location/description/svgVariant, packingList) never contains `unlocked` or checked-state; `TripState` (API response) merges it with live-read `{chapters: {id: {unlocked}}, packingChecked: {id: checked}}` from Neon, defaulting false when absent.
...
- All four API routes (`trip/[slug]`, `admin/toggle`, `push/subscribe`, `packing/check`) share one JSON envelope: `{ok: true, data}` / `{ok: false, error}`; dates are ISO 8601. `packing/check` takes an explicit desired end-state, never a bare toggle (plain write, not read-modify-write).
...
- Story 1.1's migration convention and env vars are relied on by 1.3, 1.5, 1.8, and 1.9. Story 1.2's content entry is the fixture every later story renders against.
