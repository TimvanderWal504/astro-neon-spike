...
- source_spec: none (surfaced during REVEAL.md party-mode review, 2026-09-15)
  summary: CORRECTION (2026-09-15, later same day, via Playwright): the original entry below claimed non-ASCII characters (em dashes, "é", "â", "°") come back mojibake'd through `GET /api/trip/[slug]`, based on piping `curl | python3 -m json.tool` through a Windows terminal. Once an actual headless browser was available, the same "Grândyk 2B, 9151 AE Holwert" location text rendered correctly on the live page (Ameland's own epilogue screen) — meaning the earlier "confirmed" mojibake was very likely an artifact of the terminal/python inspection path (a Windows console codepage double-decoding UTF-8), not a real bug in the deployed API or content pipeline. Downgraded from "confirmed bug" to "re-verify before trusting either conclusion" — original text kept below for the reasoning trail.
  evidence: Found while verifying the reveal's gated `revealData` field reaches the client correctly (it does) — the corruption is unrelated to that change and predates it (pre-existing fields are affected too, fields nobody touched this session). Deliberately not chased down yet — user asked to keep building REVEAL.md and come back to this before final ship. Needs root-causing against Astro's content-layer/Vercel build output before the 18th, since it affects real guest-facing text today — ORIGINAL CLAIM, now uncertain; re-check with a browser (not a terminal pipe) before acting on it either way.

- source_spec: none (surfaced during REVEAL.md build, 2026-09-15)
  summary: The existing ambient hero-map's `prefers-reduced-motion` fallback (`startViewfinderWander` in index.astro) hardcodes `translate(393px, 392px)` — the Ameland pixel-anchor within #europe-coastline — as its settled resting position, unconditionally (not gated on bestemming's unlock state at all, since it's the always-loaded ambient hero animation). Confirmed present in the built client JS bundle via grep.
  evidence: Found while auditing the new reveal work's own data flow for the same leak class (camera keypoints/LOD2-3 geometry were about to be bundled client-side before catching it — see the geodata-public/geodata-secret split in src/lib/reveal/). This is a pre-existing instance of the identical risk, not introduced this session, and a much higher bar to exploit (needs reduced-motion on, devtools, and correlating the pixel against the also-public coastline path) than a plain-text leak — but it's the same category the user called non-negotiable. Not fixed here per "keep building REVEAL.md, revisit before ship" direction; the fix is straightforward once addressed (settle on a neutral/generic point, or gate the reduced-motion fallback itself on unlock).
